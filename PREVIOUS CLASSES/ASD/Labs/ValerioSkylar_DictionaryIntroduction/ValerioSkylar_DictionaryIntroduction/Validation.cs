...
namespace ValerioSkylar_DictionaryIntroduction
{
    class Validation
    {
        ...
        public static int ValidateInt(string s, int min, int max)
        {
            ...
            {
                Console.WriteLine("Please enter a valid integer within the specified range");
                response = Console.ReadLine();
            }
        ...
        public static int ValidateIsbn(string s, int min, int max)
        {
            Console.WriteLine(s);
            ...
            while (!(int.TryParse(response, out i)) || (i.ToString().Length < min || i.ToString().Length > max))
            {
